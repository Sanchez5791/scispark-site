...
Date: 2026-05-28 (v4 base) · 2026-06-24 (v8 interactive fork)
Status: NEW v8 FORK — does NOT replace lesson-shell-v4.js. v4 stays FROZEN for the
        live lessons that load it (Y7 U1 L01 inlined · Y7 U3 L03 · Y7 U6 L01-L07).
        v8 = verbatim copy of live v4 + the 8 interactive question handlers.
        New interactive lessons (L02+) point at THIS file. Nothing live changes.

v8 ADDITIONS (per QUESTION_TYPE_MASTER_MAP §4/§9/§11 + INTERACTION_ENGINE §4):
  + 8 interactive question handlers (real, graded): matching · connect · hotspot ·
    label_diagram · draw_circuit · reorder · animation_view · interactive_explore
  + per-type partial marking (§9) + attempt rules (§11) + HINT_INTERACTIVE 3-句 (§7)
  + shared drag/drop helper (interact.js · click-to-place fallback) + bilingual render
  + sketch_graph / graph STILL BLOCKED → separate graph window (DECISIONS #9-13)
  getState() → { value, score, maxScore, attempts, correct, needsTeacher }
...
(function () {
  ...
  function voiceStartFor(field, btn) {
    ...
    // PR#73: use the student's chosen speaking language (EN/中 switch), default = page lang.
  ...
  function voiceInjectButtons() {
    ...
    Array.prototype.forEach.call(fields, function (field) {
      ...
      // Controls row: [orange Speak button] [EN | 中 speaking-language switch]
  ...
  (function initLessonShell() {
    var ALLOWED = ['lottie', 'canvas', 'svg', 'framer', 'interact', 'css',
      // v8.0 INTERACTIVE QUESTION TYPES (Spec 3 §2 · 8 non-graph types · graph stays blocked)
      'matching', 'connect', 'hotspot', 'label_diagram',
      'draw_circuit', 'reorder', 'animation_view', 'interactive_explore'];
    ...
    // ═════════════════════════════════════════════════════════════
    // v8.0 INTERACTIVE QUESTION HANDLERS — 8 non-graph types
    // Spec 3 §4 (fields) · §9 (partial marking) · §11 (attempt rules) · §7 (3-句 hint)
    // Mount contract:
    //   <div data-mount="qN" data-widget="<type>" data-options='{...JSON...}'></div>
    //   data-options → config.options (the question JSON).
    //   handle.getState() → { value, score, maxScore, attempts, correct, needsTeacher }
    // GRAPH (sketch_graph) stays BLOCKED above → separate graph window.
    // ═════════════════════════════════════════════════════════════

    // ── shared mini-helpers (self-contained · no engine internals) ──
    function _q(config) { return (config && config.options) || {}; }
    function _lang() {
      var b = document.body;
      return (b && (b.classList.contains('lang-zh') || b.classList.contains('zh-mode'))) ? 'zh' : 'en';
    }
    function _bi(en, zh) {
      return '<span class="lsv8-en">' + (en == null ? '' : en) + '</span>' +
             '<span class="lsv8-zh">' + (zh == null ? '' : zh) + '</span>';
    }
    function _node(tag, cls, html) {
    ...
    function _shuffle(arr, seed) {
      var a = arr.slice(), s = (typeof seed === 'number') ? seed : 1;
    ...
    // 3-句 hint bar (HINT_INTERACTIVE start/wrong/correct · Spec 3 §7)
    function _hintBar(opts) {
      var hints = opts.hints || {};
      var bar = _node('div', 'lsv8-hint');
      bar.style.display = 'none';
      return {
        el: bar,
        show: function (phase) {
          var en = hints[phase + '_en'], zh = hints[phase + '_zh'];
          if (!en && !zh) { bar.style.display = 'none'; return; }
          bar.className = 'lsv8-hint lsv8-hint-' + phase;
          bar.innerHTML = _bi(en || '', zh || '');
          bar.style.display = '';
        }
      };
    }
    function _checkBtn(onClick) {
      var b = _node('button', 'lsv8-check');
      b.type = 'button';
      b.innerHTML = _bi('Check', '检查');
      b.addEventListener('click', onClick);
      return b;
    }
    function _feedback() {
      var f = _node('div', 'lsv8-fb');
      ...
      return {
        el: f,
        show: function (res) {
          var full = res.score >= res.maxScore && res.maxScore > 0;
          f.className = 'lsv8-fb ' + (full ? 'lsv8-fb-right' : (res.score > 0 ? 'lsv8-fb-part' : 'lsv8-fb-wrong'));
          var mark = full ? '✓' : (res.score > 0 ? '◐' : '✗');
          f.innerHTML = mark + ' ' + _bi(
            (full ? 'Correct! ' : (res.score > 0 ? 'Partly right. ' : 'Not quite. ')) + res.score + '/' + res.maxScore,
            (full ? '正确! ' : (res.score > 0 ? '对了一部分。' : '差一点。')) + res.score + '/' + res.maxScore
          );
          f.style.display = '';
        }
      };
    }
    // shared drag/drop wiring (interact.js · click-to-place fallback · Spec 2 §3/§11 layer-4)
    // moves [data-drag] tokens into [data-zone] containers; calls onPlace(dragId, zoneId, dragEl, zoneEl)
    function _wireDrag(container, onPlace) {
      var drags = container.querySelectorAll('[data-drag]');
      var zones = container.querySelectorAll('[data-zone]');
      function place(dragEl, zoneEl) {
        if (!dragEl || !zoneEl) return;
        zoneEl.appendChild(dragEl);
        dragEl.style.transform = ''; dragEl.style.zIndex = '';
        dragEl.removeAttribute('data-x'); dragEl.removeAttribute('data-y');
        onPlace(dragEl.getAttribute('data-drag'), zoneEl.getAttribute('data-zone'), dragEl, zoneEl);
      }
      var useI = !!window.interact;
      if (useI) {
        try {
          drags.forEach(function (d) {
            window.interact(d).draggable({
              ...
              listeners: {
                move: function (ev) {
                  var t = ev.target,
                      x = (parseFloat(t.getAttribute('data-x')) || 0) + ev.dx,
                      y = (parseFloat(t.getAttribute('data-y')) || 0) + ev.dy;
                  t.style.transform = 'translate(' + x + 'px,' + y + 'px)';
                  t.style.zIndex = '999';
                  t.setAttribute('data-x', x); t.setAttribute('data-y', y);
                },
                end: function (ev) { ev.target.style.zIndex = ''; }
              }
            });
          });
          zones.forEach(function (z) {
            window.interact(z).dropzone({
              accept: '[data-drag]', overlap: 0.35,
              ondragenter: function (e) { e.target.classList.add('lsv8-zone-over'); },
              ondragleave: function (e) { e.target.classList.remove('lsv8-zone-over'); },
              ondrop: function (e) { e.target.classList.remove('lsv8-zone-over'); place(e.relatedTarget, e.target); }
            });
          });
          return;
        } catch (e) {
          console.warn('[LessonShell] v8 drag init failed → click-to-place:', e && e.message);
          useI = false;
        }
      }
      // Layer-4 fallback: click token then click zone
      var pending = null;
      drags.forEach(function (d) {
        d.addEventListener('click', function () {
          if (pending) pending.classList.remove('lsv8-sel');
          pending = d; d.classList.add('lsv8-sel');
        });
      });
      zones.forEach(function (z) {
        z.addEventListener('click', function () {
          if (!pending) return;
          place(pending, z); pending.classList.remove('lsv8-sel'); pending = null;
        });
      });
      container.classList.add('lsv8-clickmode');
    }

    // ───────────────────────────────────────────────────────────
    // 1) MATCHING — drag right token onto its left item (Spec 3: interact · P1 · partial)
    // options: { pairs:[{left:{en,zh}, right:{en,zh}}], marks, shuffle_seed, hints }
    // ───────────────────────────────────────────────────────────
    handlers.matching = function (el, config, ctx) {
      var o = _q(config);
      var pairs = o.pairs || [];
      var marks = +o.marks || pairs.length;
      var attempts = 0, placed = {};
      el.innerHTML = ''; el.classList.add('lsv8', 'lsv8-matching');
      var rows = _node('div', 'lsv8-match-rows');
      pairs.forEach(function (p, i) {
        var row = _node('div', 'lsv8-match-row');
        row.appendChild(_node('div', 'lsv8-match-left', _bi(p.left && p.left.en, p.left && p.left.zh)));
        var zone = _node('div', 'lsv8-match-zone'); zone.setAttribute('data-zone', String(i));
        row.appendChild(zone);
        rows.appendChild(row);
      });
      var bank = _node('div', 'lsv8-bank');
      _shuffle(pairs.map(function (p, i) { return i; }), o.shuffle_seed).forEach(function (i) {
        var tok = _node('div', 'lsv8-tok', _bi(pairs[i].right && pairs[i].right.en, pairs[i].right && pairs[i].right.zh));
        tok.setAttribute('data-drag', String(i)); tok.tabIndex = 0;
        bank.appendChild(tok);
      });
      el.appendChild(rows); el.appendChild(bank);
      _wireDrag(el, function (dragId, zoneId) { placed[zoneId] = dragId; });
      var fb = _feedback(), hint = _hintBar(o);
      var btn = _checkBtn(function () {
        var correct = 0;
        pairs.forEach(function (p, i) { if (placed[i] === String(i)) correct++; });
        var score = Math.round(correct / pairs.length * marks);
        if (correct < pairs.length) attempts++;
        fb.show({ score: score, maxScore: marks, correct: correct === pairs.length });
        hint.show(correct === pairs.length ? 'correct' : 'wrong');
      });
      ...
      return {
        ...
        getState: function () {
          var correct = 0; pairs.forEach(function (p, i) { if (placed[i] === String(i)) correct++; });
          return { value: placed, score: Math.round(correct / pairs.length * marks), maxScore: marks, attempts: attempts, correct: correct === pairs.length, needsTeacher: false };
        },
    ...
    // ───────────────────────────────────────────────────────────
    // 2) CONNECT — click node A then node B to draw a line (Spec 3: interact+svg · P1 · partial; extra line −0.5)
    // options: { nodes:[{id,label:{en,zh},side:'L'|'R'}], edgesExpected:[[a,b]], marks, hints }
    // ───────────────────────────────────────────────────────────
    handlers.connect = function (el, config, ctx) {
      var o = _q(config);
      var nodes = o.nodes || [], expected = o.edgesExpected || [];
      var marks = +o.marks || expected.length;
      var attempts = 0, edges = [], sel = null;
      el.innerHTML = ''; el.classList.add('lsv8', 'lsv8-connect');
      var board = _node('div', 'lsv8-connect-board');
      var svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
      svg.setAttribute('class', 'lsv8-connect-svg');
      board.appendChild(svg);
      var colL = _node('div', 'lsv8-col lsv8-col-l'), colR = _node('div', 'lsv8-col lsv8-col-r');
      var byId = {};
      nodes.forEach(function (n) {
        var b = _node('button', 'lsv8-cnode', _bi(n.label && n.label.en, n.label && n.label.zh));
        b.type = 'button'; b.setAttribute('data-id', n.id); byId[n.id] = b;
        b.addEventListener('click', function () { onPick(n.id, b); });
        (n.side === 'R' ? colR : colL).appendChild(b);
      });
      board.appendChild(colL); board.appendChild(colR);
      el.appendChild(board);
      function key(a, b) { return [a, b].sort().join('::'); }
      function onPick(id, btn) {
        if (sel === null) { sel = id; btn.classList.add('lsv8-sel'); return; }
        if (sel === id) { sel = null; btn.classList.remove('lsv8-sel'); return; }
        var k = key(sel, id);
        if (byId[sel]) byId[sel].classList.remove('lsv8-sel');
        var existing = edges.filter(function (e) { return e.k === k; })[0];
        if (existing) { edges = edges.filter(function (e) { return e.k !== k; }); drawAll(); }
        else { edges.push({ a: sel, b: id, k: k }); drawAll(); }
        sel = null;
      }
      function center(node) {
        var r = node.getBoundingClientRect(), pr = board.getBoundingClientRect();
        return { x: r.left + r.width / 2 - pr.left, y: r.top + r.height / 2 - pr.top };
      }
      function drawAll() {
        while (svg.firstChild) svg.removeChild(svg.firstChild);
        edges.forEach(function (e) {
          var c1 = center(byId[e.a]), c2 = center(byId[e.b]);
          var ln = document.createElementNS('http://www.w3.org/2000/svg', 'line');
          ln.setAttribute('x1', c1.x); ln.setAttribute('y1', c1.y);
          ln.setAttribute('x2', c2.x); ln.setAttribute('y2', c2.y);
          ln.setAttribute('class', 'lsv8-edge');
          svg.appendChild(ln);
        });
      }
      var expSet = expected.map(function (e) { return key(e[0], e[1]); });
      var fb = _feedback(), hint = _hintBar(o);
      function grade() {
        var have = edges.map(function (e) { return e.k; });
        var right = have.filter(function (k) { return expSet.indexOf(k) !== -1; }).length;
        var extra = have.filter(function (k) { return expSet.indexOf(k) === -1; }).length;
        // keep half-point granularity so each extra line really costs −0.5 (Spec 3 §9)
        var score = Math.max(0, Math.round((right / expected.length * marks - 0.5 * extra) * 2) / 2);
        return { right: right, extra: extra, score: score, ok: right === expected.length && extra === 0 };
      }
      var btn = _checkBtn(function () {
        var g = grade();
        if (!g.ok) attempts++;
      ...
      return {
        el: el, type: 'connect',
        destroy: function () { el.innerHTML = ''; },
        getState: function () { var g = grade(); return { value: edges.map(function (e) { return [e.a, e.b]; }), score: g.score, maxScore: marks, attempts: attempts, correct: g.ok, needsTeacher: false }; },
        setReducedMotion: function () {}
      };
    };

    // ───────────────────────────────────────────────────────────
    // 3) HOTSPOT — click image; point ± tolerance judged immediately (Spec 3: css/svg · P1 · point即判, no Submit)
    // options: { image, hotspots:[{x,y,r,label}], marks, tolerance_px, hints }   (x,y,r in % of image)
    // ───────────────────────────────────────────────────────────
    handlers.hotspot = function (el, config, ctx) {
      var o = _q(config);
      var spots = o.hotspots || [];
      var marks = +o.marks || spots.length;
      var attempts = 0, found = {};
      el.innerHTML = ''; el.classList.add('lsv8', 'lsv8-hotspot');
      var wrap = _node('div', 'lsv8-hotspot-wrap');
      var img = new Image(); img.className = 'lsv8-hotspot-img'; img.alt = o.alt || '';
      if (o.image) { img.src = o.image; img.onerror = function () { wrap.classList.add('lsv8-noimg'); }; }
      else { wrap.classList.add('lsv8-noimg'); } // no image → keep a clickable placeholder area
      wrap.appendChild(img);
      el.appendChild(wrap);
      var fb = _feedback(), hint = _hintBar(o);
      function near(px, py, s) {
        var dx = px - s.x, dy = py - s.y, r = (s.r || (o.tolerance_px ? o.tolerance_px / 4 : 6));
      ...
      wrap.addEventListener('click', function (ev) {
        ...
        var hit = null;
        spots.forEach(function (s, i) { if (near(px, py, s)) hit = i; });
        var dot = _node('span', 'lsv8-dot');
        dot.style.left = px + '%'; dot.style.top = py + '%';
        if (hit !== null && !found[hit]) {
          found[hit] = true; dot.classList.add('lsv8-dot-hit');
          wrap.appendChild(dot);
          var n = Object.keys(found).length;
          fb.show({ score: Math.round(n / spots.length * marks), maxScore: marks, correct: n === spots.length });
          hint.show(n === spots.length ? 'correct' : 'start');
        } else if (hit === null) {
          attempts++; dot.classList.add('lsv8-dot-miss');
          wrap.appendChild(dot);
          setTimeout(function () { if (dot.parentNode) dot.parentNode.removeChild(dot); }, 700);
          hint.show('wrong');
        }
      });
      el.appendChild(fb.el); el.appendChild(hint.el); hint.show('start');
      return {
        el: el, type: 'hotspot',
        destroy: function () { el.innerHTML = ''; },
        getState: function () { var n = Object.keys(found).length; return { value: Object.keys(found), score: Math.round(n / spots.length * marks), maxScore: marks, attempts: attempts, correct: n === spots.length, needsTeacher: false }; },
        setReducedMotion: function () {}
      };
    };

    // ───────────────────────────────────────────────────────────
    // 4) LABEL_DIAGRAM — drag labels onto image slots (Spec 3: interact · P1 · partial)
    // options: { image, slots:[{id,x,y}], labels:[{id,en,zh}], correctMap:{slotId:labelId}, marks, shuffle_seed, hints }
    // ───────────────────────────────────────────────────────────
    handlers.label_diagram = function (el, config, ctx) {
      var o = _q(config);
      var slots = o.slots || [], labels = o.labels || [], map = o.correctMap || {};
      var marks = +o.marks || slots.length, attempts = 0, placed = {};
      el.innerHTML = ''; el.classList.add('lsv8', 'lsv8-label');
      var stage = _node('div', 'lsv8-label-stage');
      var img = new Image(); img.className = 'lsv8-label-img'; img.alt = o.alt || '';
      if (o.image) { img.src = o.image; img.onerror = function () { stage.classList.add('lsv8-noimg'); }; }
      else { stage.classList.add('lsv8-noimg'); }
      stage.appendChild(img);
      slots.forEach(function (s) {
        var z = _node('div', 'lsv8-label-slot'); z.setAttribute('data-zone', s.id);
        z.style.left = s.x + '%'; z.style.top = s.y + '%';
        stage.appendChild(z);
      });
      el.appendChild(stage);
      var bank = _node('div', 'lsv8-bank');
      _shuffle(labels.slice(), o.shuffle_seed).forEach(function (lb) {
        var tok = _node('div', 'lsv8-tok', _bi(lb.en, lb.zh)); tok.setAttribute('data-drag', lb.id); tok.tabIndex = 0;
        bank.appendChild(tok);
      });
      el.appendChild(bank);
      _wireDrag(el, function (dragId, zoneId) { placed[zoneId] = dragId; });
      var fb = _feedback(), hint = _hintBar(o);
      function score() { var c = 0; slots.forEach(function (s) { if (placed[s.id] && placed[s.id] === map[s.id]) c++; }); return c; }
      var btn = _checkBtn(function () {
        var c = score(), ok = c === slots.length;
        if (!ok) attempts++;
        fb.show({ score: Math.round(c / slots.length * marks), maxScore: marks, correct: ok });
        hint.show(ok ? 'correct' : 'wrong');
      });
      el.appendChild(btn); el.appendChild(fb.el); el.appendChild(hint.el); hint.show('start');
      return {
        el: el, type: 'label_diagram',
        destroy: function () { el.innerHTML = ''; },
        getState: function () { var c = score(); return { value: placed, score: Math.round(c / slots.length * marks), maxScore: marks, attempts: attempts, correct: c === slots.length, needsTeacher: false }; },
        setReducedMotion: function () {}
      };
    };

    // ───────────────────────────────────────────────────────────
    // 5) REORDER — ▲▼ to order a list (touch-first, no hover; Spec 3: interact · P1 · 1 misplacement = marks−1)
    // options: { items:[{id,en,zh}], correctOrder:[id...], marks, shuffle_seed, hints }
    // ───────────────────────────────────────────────────────────
    handlers.reorder = function (el, config, ctx) {
      var o = _q(config);
      var items = o.items || [], correct = o.correctOrder || items.map(function (it) { return it.id; });
      var marks = +o.marks || 1, attempts = 0;
      var order = _shuffle(items.map(function (it) { return it.id; }), o.shuffle_seed);
      var byId = {}; items.forEach(function (it) { byId[it.id] = it; });
      el.innerHTML = ''; el.classList.add('lsv8', 'lsv8-reorder');
      var list = _node('div', 'lsv8-reorder-list'); el.appendChild(list);
      function render() {
        list.innerHTML = '';
        order.forEach(function (id, i) {
          var it = byId[id];
          var row = _node('div', 'lsv8-reorder-row');
          row.appendChild(_node('span', 'lsv8-reorder-txt', _bi(it.en, it.zh)));
          var ctrls = _node('div', 'lsv8-reorder-ctrls');
          var up = _node('button', 'lsv8-arrow', '▲'); up.type = 'button'; up.disabled = i === 0;
          var dn = _node('button', 'lsv8-arrow', '▼'); dn.type = 'button'; dn.disabled = i === order.length - 1;
          up.setAttribute('aria-label', 'Move up'); dn.setAttribute('aria-label', 'Move down');
          up.addEventListener('click', function () { order.splice(i - 1, 0, order.splice(i, 1)[0]); render(); });
          dn.addEventListener('click', function () { order.splice(i + 1, 0, order.splice(i, 1)[0]); render(); });
      ...
      render();
      var fb = _feedback(), hint = _hintBar(o);
      function wrongCount() { var w = 0; order.forEach(function (id, i) { if (id !== correct[i]) w++; }); return w; }
      function score() { var w = wrongCount(); if (w === 0) return marks; if (w <= 2) return Math.max(0, marks - 1); return 0; }
      var btn = _checkBtn(function () {
        var w = wrongCount(), ok = w === 0;
        if (!ok) attempts++;
      ...
      return {
        el: el, type: 'reorder',
        destroy: function () { el.innerHTML = ''; },
        getState: function () { return { value: order.slice(), score: score(), maxScore: marks, attempts: attempts, correct: wrongCount() === 0, needsTeacher: false }; },
        setReducedMotion: function () {}
      };
    };
    ...
    // ───────────────────────────────────────────────────────────
    // 7) ANIMATION_VIEW — watch animation, NOT graded (Spec 3: lottie/canvas · 不打分 · still 3-句 hint per 锁 #23)
    // options: { duration, autoplay, loop, asset, alt, caption:{en,zh}, hints }
    // reduced-motion (Spec 2 §8): ×3 duration · no autoplay · single play · not a static image.
    // ───────────────────────────────────────────────────────────
    handlers.animation_view = function (el, config, ctx) {
      var o = _q(config);
      var rm = !!(ctx && ctx.reducedMotion);
      var dur = (+o.duration || 4) * (rm ? 3 : 1);
      var loop = rm ? false : !!o.loop, playing = false;
      el.innerHTML = ''; el.classList.add('lsv8', 'lsv8-anim');
      var stage = _node('div', 'lsv8-anim-stage');
      var ball = _node('div', 'lsv8-anim-demo'); stage.appendChild(ball);
      if (o.caption) stage.appendChild(_node('div', 'lsv8-anim-cap', _bi(o.caption.en, o.caption.zh)));
      el.appendChild(stage);
      function play() {
        playing = true; ball.style.animationDuration = dur + 's';
        ball.style.animationIterationCount = loop ? 'infinite' : '1';
        ball.classList.remove('lsv8-anim-run'); void ball.offsetWidth; ball.classList.add('lsv8-anim-run');
        hint.show('correct');
      }
      var ctrls = _node('div', 'lsv8-anim-ctrls');
      var playBtn = _node('button', 'lsv8-play', _bi('▶ Play', '▶ 播放')); playBtn.type = 'button';
      playBtn.addEventListener('click', play); ctrls.appendChild(playBtn);
      el.appendChild(ctrls);
      var hint = _hintBar(o); el.appendChild(hint.el); hint.show('start');
      if (o.autoplay && !rm) setTimeout(play, 200); // no autoplay under reduced-motion (Spec 2 §8)
      return {
        el: el, type: 'animation_view',
        destroy: function () { el.innerHTML = ''; },
        getState: function () { return { value: { played: playing }, score: 0, maxScore: 0, attempts: 0, correct: null, needsTeacher: false }; },
        setReducedMotion: function (b) { if (b) { loop = false; dur = (+o.duration || 4) * 3; } }
      };
    };
    ...
    handlers.interactive_explore = function (el, config, ctx) {
      ...
      params.forEach(function (p) {
        ...
        var row = _node('div', 'lsv8-explore-row');
        row.appendChild(_node('label', 'lsv8-explore-lbl', _bi(p.label && p.label.en, p.label && p.label.zh)));
        var s = _node('input', 'lsv8-explore-slider');
        s.type = 'range'; s.min = p.min; s.max = p.max; s.step = p.step || 1; s.value = vals[p.id];
    ...
    // ── core API: getState(slot) → { type, value, attempts, timestamp [+ v8 scoring] } ──
    // v8 ADD: surface score/maxScore/correct/needsTeacher when a handler reports them
    // (additive — v4 lessons that only read value/attempts are unaffected).
    function getState(slot) {
      ...
      if (inner.score != null) out.score = inner.score;
      if (inner.maxScore != null) out.maxScore = inner.maxScore;
      if (inner.correct != null) out.correct = inner.correct;
      if (inner.needsTeacher != null) out.needsTeacher = inner.needsTeacher;
      return out;
    }
    ...
    window.LessonShell = {
      ...
      _mounted: mounted,
      version: 'v8.0.0-interactive',
      interactiveTypes: ['matching', 'connect', 'hotspot', 'label_diagram', 'draw_circuit', 'reorder', 'animation_view', 'interactive_explore']
    };
  })();
