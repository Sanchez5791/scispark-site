...
(function () {
  ...
  // ═══════════════════════════════════════════════════════════════
  // resolveActiveChild — 身份修复 (Engine-thaw Item2, 2026-07-01)。同 v8 那份逻辑。
  // 认「这次上课是哪个孩子」的 ★唯一入口★, 取代旧的 children…limit(1) 猜第一个。
  //   • 网址 ?child_id= 且确是这家长的孩子 → 用它。 • 没带 + 只有一个孩子 → 用那个。
  //   • 没带 + 多个孩子 → UNRESOLVED (★不猜★)。 • 带了但不是这家长的 / 没孩子 / 没登录 → UNRESOLVED。
  // ★救命铁律: 课堂数据(逐题/进度) 认不出 → 跳过不写; 危机/申诉(review_requests) 认不出 → 仍照写 child_id=null。★
